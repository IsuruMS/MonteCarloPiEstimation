...
long insideCircle = 0;      // number of points inside the circlr 
long totalPoints = 0;      // number of points generated
int pointsPerThread;        //number of points to be created in a thread

pthread_mutex_t lock = PTHREAD_MUTEX_INITIALIZER;

void *Algorithm()   //algorithm to generate random points and calculate how many are inside 
{

    long insideCirclrInThread = 0;   
    long pointsInThread = 0;
    unsigned int points = rand();       //random numbers generator
...
{
    if (argc != 3) {
        fprintf(stderr, "usage: ./<compiled_name> <initial no. of threads> <initial no. of points>\n");
        exit(1);
    }

    int threadsNum = atoi(argv[1]);
    long initTotalPoints = atol(argv[2]);

    //files to write the outputs
    FILE *outFile;
    outFile = fopen("results1.txt" , "w+");
    FILE *outFile1;
    outFile1 = fopen("TotalPoints1.txt" , "w+");
        ...
        {        
            ...
            pointsPerThread = initTotalPoints/threadsNum;
            ...
            free(threads);      //free the dynamic space allocated for threads
    ...
    printf("Done. Check the files results1.txt and TotalPoints1.txt made in the directory.\n");
}